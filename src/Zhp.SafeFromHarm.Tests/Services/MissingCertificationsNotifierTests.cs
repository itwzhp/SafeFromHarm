...
public class MissingCertificationsNotifierTests
{
    ...
    public async Task SendNotificationsOnMissingCertificates_FindsRequiredMembers()
    {
        ...
        senderSubstitute.ReceivedCalls().Single(c => c.GetArguments().First() as string == "hufiec@zhp.example.com").GetArguments().Should().SatisfyRespectively(
            ...
            p => p.As<IEnumerable<MemberToCertify>>().Should().BeEquivalentTo(new MemberToCertify[] { new("Jan", "Kowalski", "AA02", new(10, "Hufiec", "hufiec@zhp.example.com")), new("Anna", "Nowak", "AA03", new(10, "Hufiec", "hufiec@zhp.example.com")) }),
            p => p.As<IEnumerable<(MemberToCertify, DateOnly)>>().Should().ContainSingle().Which.Should().Be((new MemberToCertify("Jan", "Kowalski", "AA01", new(10, "Hufiec", "hufiec@zhp.example.com")), DateOnly.FromDateTime(DateTime.Today).AddDays(-10))));
        ...
        senderSubstitute.ReceivedCalls().Single(c => c.GetArguments().First() as string == "drugihufiec@zhp.example.com").GetArguments().ElementAt(2)
            .Should().BeEquivalentTo(new MemberToCertify[] { new("Tomasz", "Innyhufiec", "AB01", new(11, "Drugi Hufiec", "drugihufiec@zhp.example.com")) });
    }
